...
**In progress:** v10.0 Eval & Trace Observability + Hardening — Phase 94 (Trace Waterfall) complete 2026-07-06: `traceId` grouping live end-to-end on prod (Ástríðr per-turn contextvar at `_process_inner` → all 3 provider `llm_call` emits → `/runtime-ingest` alias → `llmMetrics.traceId` → `sessionCalls` query → Gantt-styled `TraceWaterfall` on SessionDetail's new Trace tab with `?tab=trace` deep-link + Analytics "Recent LLM Calls" cross-link; dead `LangfuseTraceLink.tsx` deleted). Verified 22/22 must-haves + operator live sign-off (grouped turn render, legacy untraced fallback, clean console); WR-01 cache-ratio review finding fixed same-day. Phase 93 (Eval Pipeline & Quality KPIs) complete 2026-07-06: `evalScores` pipeline live end-to-end on prod, verified 18/18. Next: Phase 95 (Hardening — security audit, key rotation, dependency majors). Known follow-up: war-room containers still run the pre-94 image and emit untraced calls until rebuilt.
