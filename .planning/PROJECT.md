...
**v16.0 Phase 128 (Planning Reconciliation) COMPLETE (2026-08-27):** 5/5 plans across 3 waves, verification `passed` 4/4 (RECON-01..04), code review 0 Critical / 3 Warning. The tracker now matches the code: 6 todos closed with `file:line` evidence, 2 kept open because re-derivation caught the scoping sweep overclaiming (`convex/metrics.ts:19`'s unbounded `.collect()` is still live and became FIX-01), 6 deferred as `REQUIRES LIVE MEASUREMENT` to named phases rather than guessed at with a static proxy, and all 9 seeds reconciled so `/gsd-new-milestone` can no longer re-propose work already on the v16.0 roadmap. The nine-item carried-forward list is dissolved, one of its rows corrected on re-derivation (Phase 117 is PARTIAL, not closed). **The gates found more than the work did.** An adversarial gate (mutation tester + claims auditor per plan; a runtime prover on the first three) returned **11 real defects** and a code review **3 more** — all 14 fixed, each proven red-then-green against a control that could have come out the other way. Every plan's own SUMMARY had said Self-Check PASSED. **Five were guards that could not fail on the defect they were written to catch:** `closed-todos.mjs` captured a citation's line number and discarded it, so `ForgeJobList.tsx:999999` validated against a 309-line file; `open-todos.mjs` silently reclassified a corrupted deferral as evidence-backed and had no zero-population guard (its own threat register named that class and covered two of three populations); `seed-status.mjs` let a genuinely absorbed seed revert to `dormant` — the exact re-proposal failure the plan exists to prevent; and `gitIsShallow` had no test at all, so neutering it to `return false` left the suite fully green. The code review then caught a live logic bug in the new ratchet: commit identity was decided by string equality while `completionCommitFor` returns a full 40-char SHA and the file's own documented stamp convention is 7 chars, so the most common FRESH case — re-deriving at phase close and stamping it as documented — would have reported STALE. **Stated honestly rather than rounded:** RECON-04's live assertion is vacuous today (46 `Pending` rows, 0 `Partial`); all discrimination is carried by an injected fake-oracle table, real-git fixtures, and one live red/green mutation against the real planning files with a byte-identical restore, and deleting any of those three silently guts the guard. `open-todos.mjs` carries a stated LIMIT: every Re-derivation section closes with a boilerplate `.planning/REQUIREMENTS.md:NNN` self-citation that always resolves, so breaking the real code citation alone does not go red — banning planning-internal citations was rejected as false precision, because for `public-repo-exposes-…` that line genuinely IS the evidence (its claim is an absence). **Durable mechanisms added, not just fixes:** all three `checks/*.mjs` now run under `npm test` and CI via `src/planningChecks.ratchet.test.ts` (they previously ran only when a human remembered), and `.github/workflows/ci.yml` gained `fetch-depth: 0` — guarded by a js-yaml-parsing test, since nothing had guarded that single line and its deletion was silent to the whole suite. **Tooling caveat, again:** `gsd-sdk query phase.complete 128` returned `warnings: []` while leaving `completed_phases` at 0, `percent` at 0, and the ROADMAP Progress row at `0/5 | Not started` directly beneath the checkbox it had just ticked; all repaired by hand and re-derived from disk. Suite 380 files / 5398 tests passed, browser 3/3, no product code touched.
**v15.0 Phase 121 (Analytics Query Resilience) COMPLETE (2026-08-18):** 7/7 plans across 4 waves, verification `passed` 3/3 criteria and 6/6 must-haves, code review **clean — 0 findings**. DEBT-08 closed. `/analytics` no longer fetches anything in its own page component: ten hoisted `useQuery` calls moved into eight self-fetching children, each behind its own `SectionErrorBoundary`, and the previously unguarded Summary Row now has four. The dead `llm.subscriptionUsage` read that caused the 2026-08-11 blackout is gone. **What actually proved the goal was not the injected tests.** During the live smoke check three REAL un-injected queries were timing out server-side (`analytics:activityHeatmap`, `toolFlowSankey`, `tokenSunburst`) and each was caught by its own boundary while every other panel kept rendering — three simultaneous failures, zero collateral, where pre-121 any one would have blanked the route. `costByModel`/`providerBreakdown` now read the `aggregates` rollups (0 non-comment `.collect()` left in `llm.ts`), and `costByProvider`, `latencyOverTime` and `useLatencyOverTime` were deleted outright behind a blocking operator checkpoint, with the deletion confirmed against the backend's own function enumeration. `ROLLUP_READ_CAP` was **measured** live at 889/1773 rows read (11%/22% of 8000) and left unchanged, closing a `121-RESEARCH.md` question that had been flagged and never taken. A D-04 AST ratchet guards the page structurally: it derives its candidate set from the AST rather than enumerating the eight component names, and it was mutation-proven against a synthetic component absent from the whole repo — the enumerated-guard failure mode that stayed green through five identical defects elsewhere in this project. **Two process findings worth carrying into 122's planning.** First, FIVE of seven plans shipped acceptance criteria that were provably wrong while their intent was right — a literal `metric_type: "calls"` grep the plan's own mandated helper can never emit (0 for the already-shipped `tokens_prompt` too), a self-contradictory `tsc` scope, an assertion that passed against a deliberately mutated component, a grep count already wrong at baseline, and a loop-vs-literal-count gap. Every executor caught its own and replaced it with a discriminating check; nothing shipped on a vacuous green, but the planning step produced bad criteria at a 5-in-7 rate. Second, `npx convex deploy` is refused by the Claude Code auto-mode permission classifier, so **an agent structurally cannot deploy this project** — the operator ran it, and any future phase touching `convex/` should plan the deploy as an attended step rather than discovering the block mid-wave. **Carried forward, not blocking:** `analytics:activityHeatmap`, `toolFlowSankey`, `tokenSunburst` and `metrics:dashboardSummary` are unbounded `.collect()` scans that now time out server-side at current volume — filed as `todos/pending/unbounded-analytics-scans-timeout.md`, scoped as a triage keyed on table size and explicitly NOT a sweep of all 218 non-comment `.collect()` sites, most of which are over small or index-bounded tables. Also latent and untriggered: `aggregates.ts`'s backfill worst-case read count can exceed Convex's 4,096-READ ceiling (pre-existing since Phase 104; ~7 rows/hour actual). Suite 4772 passed | 0 failed, tsc 0 errors, build exit 0.
...
---
*Last updated: 2026-08-27 — **v16.0 Phase 128 (Planning Reconciliation) COMPLETE**: 5/5 plans, 4/4 requirements verified, 14 defects found by the adversarial gate and code review and all fixed with red/green proofs. First phase of v16.0 "Terminal Zero" (Phases 128-157). Next: Phase 129 (Dashboard Unbounded Read & Ratchet Coverage), which owns the still-live `convex/metrics.ts:19` unbounded `.collect()` this phase refused to close.*
