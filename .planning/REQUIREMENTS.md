...
- **BSC-01** — Live view of the **current reasoning engine** per agent AND globally, reactive from Convex/telemetry (not a stale config read — the v9.0 "VitalsRail active-profile" trap). — ⚠ **NOT satisfied — gap closure required** (marker corrected 2026-07-28 after live verification; see `103-VERIFICATION.md` / `103-VALIDATION.md`). Genuinely delivered: the `Settings.tsx` stale `p.model` config read is deleted, and the reactive `activeEngineSnapshots` substrate is live. But the "AND globally" half **fails on the running stack**: because the badge, composer pill, and Settings row read *exclusively* through `useActiveEngine()` (the per-profile axis), none of them can observe an active global override. Observed live with `claude-haiku-4-5-20251001` genuinely in force: badge showed `No brain reported`, composer pill showed `Auto`, and only the pre-existing `BrainControl` was correct — three surfaces, three answers, which is the exact trap this requirement exists to close.
- **BSC-02** — **Switch the engine on the fly**: choose from keyed API models (Claude family — Opus 4.8 / Sonnet 5 / Fable 5 / Haiku 4.5 — and any others astridr exposes) and subscription CLIs (Claude Code / Codex / Antigravity); dispatched to astridr's brain-swap endpoint via authenticated `/api/*` (bearer per CLAUDE.md). — ⚠ **partial — gap closure required** (marker corrected 2026-07-28 after live verification; see `103-VERIFICATION.md`). Genuinely delivered: `BrainPicker` is reachable from all three hosts, lists **331 real engines** from the live `swap.catalogue`, and the global branch's `swap.set` dispatch is confirmed to change the running brain end-to-end. Per-profile branch remains stub-backed pending astridr Phase 184.1 (by design, not a defect). **Outstanding:** `BrainPicker.tsx:414-419` never wires `onSelect` on its `CommandItem`s, so the component's own designed primary flow — search → arrow-navigate → Enter — is entirely non-functional; swapping works by mouse only.
