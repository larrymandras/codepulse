...
- [ ] **TOKEN-02** — The three-hue-owner law holds app-wide: cyan = machine, violet = Ástríðr **only**, `--status-*` = state. This includes decoupling `--status-ok` from `--primary`. *(Corrected 2026-08-18 during Phase 122 discussion, per the Stale Docs rule: this line previously cited `index.css:139/165`, which is wrong twice over — `:139` is `--card-foreground` and `:165` holds no status token, and the collision is in **three** themes, not one. Measured live: `cyan` `:142`/`:171` both `#06b6d4`; `emerald` `:198`/`:205` both `#10b981`; `amber` `:220`/`:227` both `#f59e0b`. Already decoupled in `readable` (`#5eead4` vs `#34d399`) and `aubergine` (`#c084fc` vs `#34d399`). Note `emerald`'s `--primary` is itself a green, so a single sea-green `--status-ok` cannot satisfy this requirement there — see `122-CONTEXT.md` D-05.)*
