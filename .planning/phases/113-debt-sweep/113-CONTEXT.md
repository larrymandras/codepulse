...
- **D-17 [ADDED 2026-08-11 after research]: The frontend origin-coupling fix ships INSIDE DEBT-05.** D-02's new plugin origin is not complete until the surfaces that read origins accept it. Verified in this session, non-test production sites that match the old origin *exactly*: `src/pages/Skills.tsx:132` (Global tab filter) and `:148` (Global tab count) — both `.includes("claude-code")`, an exact match that a `claude-code:plugin` row fails; `src/lib/skills.ts:69` (`moveDestinationIsProject`), `:125` (`isActiveGlobal`), `:131` (hardcoded `sourceOrigin`), `:165` (`originLabel`); `src/components/skills/SkillLifecycleMenu.tsx:88` (origin → "global"); and `src/components/OriginBadge.tsx:6` (badge style map — a 4th file, not named in the research summary). `src/lib/skillVault.ts:12` is **safe by design** and needs no change: it maps *anything else* to "global", so a new origin falls through correctly.

  Shipping D-02 without these silently drops ~57 plugin skills from the Skills page's Global tab and its count, and costs them drag-and-drop — i.e. DEBT-05 would "close" by making the catalog visibly wrong in a different way. This is a required consequence of D-02, not new scope, and it overrides this phase's "no UI work" framing (which was written before the coupling was known). **Larry's call, 2026-08-11.** Rejected: deferring it to a follow-up phase (leaves the Skills page wrong between phases) and extracting a shared `isGlobalOrigin()` predicate now (tempting, and it would kill the defect class, but it widens a debt-sweep phase into a refactor — the 4 sites are mechanical and already have test coverage).
...
- **D-08 [AMENDED 2026-08-11 after research]: Capture = retained soak output AND failure-time DOM instrumentation captured AT THE QUERY SITE.** The failing assertion is `expect(labelBefore).toBe("anthropic-sonnet-5")` (`src/pages/Chat.test.tsx:585-586` — re-derived 2026-08-11; the deferred-item doc's `:576-577` is stale, the file shifted after Phase 111-02 edited it), a plain string `toBe`.

  **Confirmed by the mandated control:** research ran a deliberately-mismatched expected value at the exact assertion site and captured `Expected: "DELIBERATE_CONTROL_MISMATCH_9x7q2"` / `Received: "anthropic-sonnet-5"` (`113-RESEARCH.md` Control 1, file restored byte-exact afterward). So the actual `textContent` **is** printed and was merely not retained in the original run — the deferred item's "unobtainable" framing is wrong.

  **Falsified by a second control — this is the amendment:** the original decision specified an `onTestFailed` hook in `src/test/setup.ts`. That mechanism captures nothing. `@vitest/runner`'s per-test finalization order is fixed and sequential — `afterEach` (1st) → fixture cleanup → `onTestFinished` → `onTestFailed` (LAST) — and `@testing-library/react` auto-registers `afterEach(() => cleanup())` at import time because `vitest.config.ts:14` sets `globals: true`. `cleanup()` unmounts every tree and strips the DOM, so a global `onTestFailed` dump always sees an empty `<body>`. Proven both from runner source and empirically. This ordering is fixed control flow, **not** configurable via `sequence.hooks`.

  **Therefore:** capture DOM state synchronously at the point of query/assertion — a `try`/`catch` or an inline `queryAllByTestId` — never via a post-hoc global hook. Retained output alone is still insufficient: the leading remaining hypothesis (a stale element surviving from an earlier render) turns on whether *two* elements matched the testid, which a string diff cannot show; an inline `queryAllByTestId` can. **Instrumentation acceptance test:** the first captured dump must come from a deliberate control and be NON-EMPTY. An empty dump is a bug in the instrumentation, never evidence the DOM was empty at test time.
...
- **D-18 [ADDED 2026-08-11 after research]: `INSTANCE_SECRET` is PARAMETERIZED before the first commit — not rotated in this phase.** Verified this session: `C:\Users\mandr\convex-selfhost\docker-compose.yml:89` carries the live secret inline as `- INSTANCE_SECRET=<value>`, and that file sits squarely inside D-13's committed scope while D-14's secrets scope names only `admin-key.txt` and `selfhosted.envfile` — so this value is uncovered and would be committed in the clear. Move it to the ignored env file and commit the compose with a variable reference; the `.example` template gains the key NAME only, per D-14's build-from-documented-names rule.

  **Rotation is explicitly NOT in this phase.** Research flags that the secret is already mirrored to NAS in plaintext by `backup-convex.ps1`, which is a real exposure — but rotating `INSTANCE_SECRET` on the live self-hosted backend is a production operation on a stack `CLAUDE.md` documents as memory-pressured and volume-sensitive, and it is far beyond "put this directory under version control." **Larry's call, 2026-08-11:** parameterize only. The NAS plaintext exposure is recorded here so it is not silently dropped, but it is not actioned by Phase 113. Rejected: parameterize-and-rotate (correct end state, wrong phase) and leaving it inline (commits a live secret).
...
- ~~The soak iteration budget for D-09/D-10.~~ **DECIDED 2026-08-11 (Larry): tiered — 30 full-suite iterations, then 50 more only if the first tier is clean.** Grounded in a measurement taken this session, not an estimate: one full `npx vitest run` = **37.98s** (298 files passed, 3958 tests, 0 failures), so tier 1 ≈ 19 min and the worst case ≈ 51 min. Stop early and immediately on any reproduction. Note there is **no `--repeat` CLI flag** on the installed Vitest (research confirmed via a known-invalid-flag control), so this is a shell loop. Per the Validation Architecture, each iteration's pass/fail must be recorded individually (append per-iteration to a log) — a failure on iteration 23 of 30 must not be overwritten by iteration 30's success. Rejected: flat 100 (~63 min, burns the full hour even when a reproduction lands early) and tiered 50→100 (~95 min, more confidence than a once-in-a-dozen defect warrants).
