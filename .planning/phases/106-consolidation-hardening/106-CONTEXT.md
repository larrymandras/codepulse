...
- **D-02:** Export the ~56 GB pre-2026-07-15 cloud history to **local disk on this office PC** (scratch path), verify the archive is complete/readable, then it can be deleted once confirmed — no requirement to retain it long-term or move it to G:\ cold storage.
  - **CORRECTED 2026-08-05 (Phase 106, plan 106-03 execution, Stale Docs rule): the "~56 GB" is wrong and belongs to a different instance.** The real export of `tidy-whale-981` is **617 MB compressed / 2.0 GiB uncompressed / 602,932 rows across 80 non-empty tables**, taken in ~6 minutes. The 56 GB figure traces to the *self-hosted* Convex incident (2026-07-17…22), where a full snapshot export peaked at ~56 GB of transient scratch on a ~1M-doc DB — a different deployment, and a peak rather than an archive size. Flagged because a 2%-of-budget result looks exactly like a truncated export: completeness was proven separately (full-archive CRC, per-table row census, events span 2026-05-21 → 2026-07-15T00:04:25Z). Evidence in `106-DEBT-VERIFICATION.md` § "DEBT-02 — Export & verification". Everything else in D-02 stands.
