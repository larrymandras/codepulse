...
| Per budget that reaches a non-null classification: `alerts.by_source` dedup read | Range-bounded to the CURRENT period: `q.eq("source", …).gte("createdAt", periodStart)` — at most the alerts fired for this budget/level within this one period | Corrected in `527adc7f` (orchestrator review). As first written the read had no lower bound, so it read every alert ever fired for that `cost-budget:{budgetId}:{level}` source; `alerts` is excluded from `retention.ts`'s `RETENTION_DAYS` ("kept forever"), so it grew one row per fired period without limit — the same read-growth shape that got `internal.alerts.evaluateInternal` disabled on 2026-07-14. The composite `["source", "createdAt"]` index makes the bound free, and it cannot drop a match (an alert for period P is written during P, so `createdAt >= P`). |
