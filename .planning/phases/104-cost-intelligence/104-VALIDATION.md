...
| D-03: the "N models need rates" nudge reflects reality on day one | COST-01 | **FAILED (gap)** | Passed at 19:00Z when `unpricedModels` returned `{count: 0}`; **re-checked on the rendered UI at 2026-08-02 and it now FAILS.** The nudge itself is faithful — it renders 4 and the 24h query returns exactly 4 — so the defect is in `costDerived.unpricedModels`, not the component. Two problems: (a) it lists **`claude-cli`, which IS priced** — `costBreakdown` returns that same model as `priced: true, pricedVia: "model", coveredUsd: 0.180785` against the seeded D-06 shadow row, so two queries in one module disagree about the same model; (b) all four entries carry `promptTokens: 0, completionTokens: 0`, so the copy "their cost isn't in the total above" implies missing money that does not exist. The set is also window-unstable: 24h -> 4 (claude-cli, codex, antigravity, claude-sdk, all zero-token), 168h -> 3, 720h -> 6 (three with real tokens: gpt-4.1, grok-4.5, google/gemini-3.6-flash). Most likely surfaced by this phase's own backfill creating zero-token bucket dimensions. Routed to the gap plan. |
...
| 4 | `llm:costOverTime` and its `useCostOverTime` hook have ZERO consumers since 104-10 moved `CostTrendChart` onto `costDerived.costOverTime` — a second dead unbounded 30-day scan. | Routed to gap plan. |
| 5 | `costDerived.unpricedModels` names a priced model and counts zero-token models (see D-03 row). | Routed to gap plan. |
...
- [~] Every **Manual-Only** row above executed and recorded — **5 of 7 PASS** (D-14, A1, D-18, D-20, plus D-04's mechanism); **D-03 FAILED** on re-check; D-16 PASSED on the live alert; step 5 partial. All recorded verbatim, none inferred from a green suite.
- [ ] `nyquist_compliant: true` set in frontmatter

**Approval:** NOT APPROVED (updated 2026-08-02). The live pass closed D-18/D-20/A1/D-16/D-15 and the theme sweep, but surfaced a D-03 accuracy failure plus 5 defects (3 fixed in-session, 2 routed). `nyquist_compliant` stays false and Phase 104 stays OPEN pending `/gsd-plan-phase 104 --gaps`. — Task 1 (deploy/seed/backfill) complete and recorded; D-18/D-20/A1 deferred
