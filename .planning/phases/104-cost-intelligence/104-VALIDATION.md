...
| D-18 covered-vs-billed on screen | The gateway turn landed at ~19:31 UTC, inside an incomplete hour; `computeHourly` only rolls up the last COMPLETE hour, so the shadow-priced `coveredUsd` cannot appear until after 20:00 UTC. The hourly cron will do this unattended. Re-check with `npx convex run costDerived:costBreakdown '{"period":"hourly","lookbackHours":3}'` and confirm the `claude-cli` row shows `coveredUsd` populated and `billedUsd` zero. | **PENDING ROLLUP** |
