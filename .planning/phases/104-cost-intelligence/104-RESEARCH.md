...
| V4 Access Control | yes | `costBudgets`/`modelPricing` mutations are internal Convex functions reachable only through the existing dashboard's Convex client — mirror `alertRuleCustom.ts`'s existing pattern (public `mutation()`, no additional identity check beyond what the rest of the admin surfaces use; this project's Clerk gate is optional per CLAUDE.md, so do not introduce a stricter bar than existing sibling admin surfaces without an explicit decision) |
...
|---------|--------|---------------------|
| Unauthenticated write to a new public `mutation()` (e.g. `costBudgets.create`) | Tampering | Follow the exact same pattern already accepted for `alertRuleCustom.create`/`forecasts.setBudgetCap` in this codebase — do not introduce a bespoke auth check that diverges from the sibling admin surfaces without an explicit decision, since that inconsistency is itself a maintenance/security-review burden |
...
| A3 | No operator-elevated auth check beyond the existing project convention is required for the new `costBudgets`/`modelPricing` mutations | Security Domain | If Larry wants a stricter bar (e.g. Clerk-required) for money-affecting config specifically, this needs an explicit decision — currently unstated in CONTEXT.md's Decisions or Discretion sections |
