...
| 94-04-T2 | 04 | 3 | TRACE-02 | — | LangfuseTraceLink deletion clean; TokenSavingsIndicator preserved | smoke | `rg "LangfuseTraceLink" src/` (zero) + `npx tsc --noEmit && npm run build` | ✅ existing cmds | ⬜ pending |
| 94-04-T3 | 04 | 3 | TRACE-02 | T-94-08, T-94-10 | encodeURIComponent + sessionId guard; JSX-escaped model/time; bounded pagination; token-only accent | smoke | `rg "View Trace" src/pages/Analytics.tsx` + `rg "\?tab=trace" src/pages/Analytics.tsx` + `npx tsc --noEmit && npm run build` | ✅ existing cmds | ⬜ pending |
...
*Existing vitest + pytest infrastructure covers all phase requirements — no framework install needed. Plan 04 tasks are smoke-verified (tsc + build + rg) — no net-new test files.*
...
| Recent LLM Calls "View Trace" cross-link navigates to the correct session's Trace tab | TRACE-02 | Client-side routing under live data; unit smoke only checks presence | Plan 05 Task 2: on Analytics, click "View Trace" on a row with a sessionId; confirm it lands on `/sessions/:id?tab=trace` with the waterfall selected |
...
**Approval:** planned — 2026-07-06 (revised: 94-04 split into 3 tasks — deletion (T2) + Recent LLM Calls table with View Trace link (T3))
