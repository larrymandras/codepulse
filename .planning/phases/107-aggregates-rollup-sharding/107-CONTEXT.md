...
- Exact TypeScript shape of the shard-summing helper (e.g. a shared `sumAcrossShards(rows)` utility vs. inline `.reduce()` at each call site) — planner's call, but prefer one shared helper given 3+ call sites need the identical fold.
...
- `convex/analyticsRollupQueries.ts` — **must be read by the researcher/planner**: this is where the actual per-bucket fold/grouping logic lives for all three functions above. Not yet verified whether it already sums by `(bucket_start, dimensions)` key across multiple rows (shard-safe as-is) or assumes exactly one row per key (would silently undercount once shards exist). This is the single most important open question for the planner to resolve before writing tasks.
