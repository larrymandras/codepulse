...
2026-08-06..2026-08-10 05:30  verdict=OK  all tables caught up, no timeouts
2026-08-10 18:01:30 verdict=ALERT tables=19 ... timed out ... worst overhang 172.8h
```

Index-head timeouts are a pre-existing condition on this instance that tracks memory pressure — two
prior independent occurrences, both at 41+ GiB, both before this plan touched anything. This is not
an unexplained new failure introduced by probing 5 more tables.

Memory samples, taken by the orchestrator directly via `docker stats --no-stream`:

| Time | Memory |
|---|---|
| 05:30 today (from the health log's own OK line) | 16.02 GiB |
| 16:44 | 23.57 GiB |
| 18:00 (during this run) | 32.51 GiB |
...
between 16:44 and 18:00. The deploy (plan 110-04) and this plan's 19-probe run both sit inside that
window. **Recorded as correlation with a plausible mechanism — no experiment isolated cause, and
this is not asserted as the deploy having caused the memory jump.**

Also worth stating plainly: part of this ALERT is the check *working as designed*. The 172.8h
`aggregates` overhang is real, expected (the new predicate has not run under the nightly cron yet —
that is leg 1, plan 110-06), and was structurally invisible to every health check that ran before
tonight, because the pre-110 script never queried that table at all. A table that could not be seen
cannot be alerted on; this run is the first time it could be.

---

## Operator decisions and deliberate-break test (Task 3)

### Operator Decision 1 — how to close leg 2, recorded verbatim

> "Record it, re-run clean tomorrow"

Presented to the operator as: "Accept leg 2's coverage claim now with the TIMEOUT recorded verbatim
and the log history showing it's pre-existing and memory-correlated, NOT introduced by the edit.
Then take a clean zero-TIMEOUT run tomorrow morning after the 02:00 restart, in the same session as
110-06."

**Consequence for this leg's status:** leg 2 closes on **coverage**, which is fully proven (three-way
count cross-check equal at 19; all five previously-invisible tables confirmed visible by name). The
`events` TIMEOUT is recorded as an **open item**, explicitly not waved away — it is not resolved by
this plan. A clean zero-TIMEOUT run is **deferred to tomorrow morning**, in the same session as plan
110-06's leg 1 observation of the 09:00 UTC nightly cron. Task 2's literal "zero TIMEOUT rows"
acceptance criterion is **not** met by tonight's run; that is recorded as a deviation below, not
claimed as passed.

### Operator Decision 2 — who runs the deliberate-break test

Operator chose: "I run it, you review the output." The orchestrator ran it in the attended session
with the operator present; this executor did not run it (per this plan's `<live_instance_safety>`
bound and the checkpoint hand-off, this executor is recording the transcript, not re-executing it).

### Deliberate-break test transcript

A snapshot was taken of the **edited** script (the live, post-110 version — not restored from
`.pre-110.bak`, which holds the pre-edit original; restoring from that backup for the break test
would have silently undone the whole plan under test).

```
hash of edited script before test : 3F579801768BADD75E5E6AF4D1DD13E220E46BC3F984730C92CD1DBB1D073492
occurrences of retention:listRetentionPolicy before : 1
after injecting bogus name        : real 0 / bogus 1
```

Broken run output (head), via `powershell -NoProfile -ExecutionPolicy Bypass -File`:
...
[... CLI's available-functions list ...] -- no fallback table list used
...
```
expected 3F579801768BADD75E5E6AF4D1DD13E220E46BC3F984730C92CD1DBB1D073492
actual   3F579801768BADD75E5E6AF4D1DD13E220E46BC3F984730C92CD1DBB1D073492  -> byte-identical
real function name occurrences after restore : 1
bogus occurrences after restore              : 0
temp snapshot removed

.pre-110.bak still present            : True
Get-ScheduledTask ConvexRetentionHealthCheck : State=Ready
LastRunTime 8/10/2026 5:30:01 AM / NextRunTime 8/11/2026 5:30:00 AM  (unmodified)
```

**VERDICT: PASS.** The hard-failure path is proven to fire, not merely described. Pointing the
policy read at a nonexistent function produced the distinct policy-read-failure verdict, exited
non-zero, probed **zero tables**, and used **no fallback list** — exactly the property D-07 exists
to guarantee. The script was restored to the byte-identical edited state (hash match above), the
`.pre-110.bak` backup remains present and untouched, and the `ConvexRetentionHealthCheck` scheduled
task remains enabled with its `NextRunTime` at tomorrow 05:30, unmodified by this test.

**Incidental confirmation:** the broken run's available-functions list ends with `galdr:list`,
`galdr:lookup`, `galdr:createPrompt`, etc. — independently confirming (a second time, beyond plan
110-04's own readback) that the wave-3 deploy shipped Phase 116's galdr backend, as the operator was
told it would before authorizing that deploy.

---

*Plan 110-05 is complete: Task 1 (script edit, verified), Task 2 (DUR-02 leg 2 coverage capture),
and Task 3 (operator checkpoint — both decisions recorded, deliberate-break test proven to fire) are
all closed. The `events` TIMEOUT remains an open item, deferred to a clean re-run tomorrow morning
alongside plan 110-06.*
