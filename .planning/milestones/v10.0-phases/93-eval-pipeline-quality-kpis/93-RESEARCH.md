...
1. **How does a `sessions`/`events` row get attributed to a persona/profileId for EVAL-02's sampling query?**
   ...
   - **RESOLVED (Plan 02, Task 3 “PERSONA DECISION”):** “active persona” = `profileConfigs` rows (`personal`/`business`/`consulting`); sessions are attributed via `llmMetrics.agentId`, which astridr's `loop.py`/`insight_extractor.py` set to the active profileId at most call sites. Sessions with no resolvable attribution (e.g. the `loop.py:1537` `_agent_type` fallback) are judged under an explicit `"unknown"` bucket that is counted in the action's `N sampled / N scored / N failed` liveness summary, making attribution drift visible rather than silently absorbed.
...
2. **Which specific `configKeys` count as "persona-scoped" for regression detection (D-11)?**
   ...
   - **RESOLVED (Plan 01, Task 2):** key naming = `` `profile.${profileId}.modelPreferences` ``, written by the new `configChanges` audit insert added to `profiles.upsertConfig` (matches the `updateEmail` `profile.<id>.<field>` precedent). `agentProfiles.update` is excluded per Assumption A1 (zero rows, not the real persona mechanism).
...
3. **Should the regression detector run inline inside the nightly judge action, or as a separate scheduled action?**
   ...
   - **RESOLVED (Plan 04, Task 2):** `detectRegressions` runs as a tail call appended to `judgeSessionsAction` after the `Promise.allSettled` batch resolves — no separate cron slot, and the night's own scores are all counted before the before/after comparison runs.
