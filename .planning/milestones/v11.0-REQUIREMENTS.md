...
- [x] **INTAKE-01**: User can install a skill from an uploaded SKILL.md to a chosen scope (global / project / cold storage) and the file actually lands on the host disk — not the current "validation only, nothing written" dry-run.
- [x] **INTAKE-02**: User can install a skill from a GitHub URL (with optional `subpath` fan-out) to a chosen scope, with the existing URL-shape and path-traversal guards enforced.
- [x] **INTAKE-03**: After a successful install the skill appears in the Skills page automatically with correct origin/scope (daemon-driven registry rescan) — no manual refresh.
- [x] **INTAKE-04**: The intake outcome surfaces the daemon's real execution/validation report; a failed install reports an actionable error and leaves no partial state on disk.
...
- [x] **DAEMON-01**: The Forge daemon executes `intake` commands — validates then writes SKILL.md to the destination scope (global / project / cold) on the host filesystem.
...
- [x] **DAEMON-04**: The daemon advertises its new supported command types so older daemons never receive commands they cannot run (extends `supportedTypes` / `resolveClaimTypes`).
...
| REQ-ID | Phase | Status |
|--------|-------|--------|
| INTAKE-01 | Phase 97 | Complete |
| INTAKE-02 | Phase 97 | Complete |
| INTAKE-03 | Phase 97 | Complete |
| INTAKE-04 | Phase 97 | Complete |
| DAEMON-01 | Phase 97 | Complete |
| DAEMON-03 | Phase 97 | Complete |
| DAEMON-04 | Phase 97 | Complete |
...
**Coverage:** 22/22 v1 requirements mapped — no orphans, no duplicates. **All 22 Complete** (milestone shipped 2026-07-25).

**Note (2026-07-25, v11.0 milestone close):** The INTAKE-01..04 + DAEMON-01/04 rows were reconciled from `Pending` → `Complete` at milestone close. Phase 97 (Real Skill Intake & Daemon Foundation, 6/6 plans, operator-verified live 2026-07-19, ROADMAP `Complete`) delivered them — every one is claimed in a Phase-97 plan's `requirements-completed` frontmatter (97-01..06 SUMMARY.md); the traceability checkboxes were simply never updated during Phase 97's own close (the same stale-traceability pattern noted for v9.0/v10.0). No code gap.
