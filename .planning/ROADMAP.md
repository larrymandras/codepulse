...
| 123. Accessibility Remediation | v15.0 | 0/13 | Planned | - | Planned 2026-08-20, 13 plans across 6 waves (`945116ee`), `--skip-ui` per operator decision as with 120/121/122. Plan-checker: 0 blockers, 3 warnings, 1 info — all closed (`5c890322`, `123-PLAN-REVIEW.md`). Decision coverage 18/18 with `skipped:false`, proven to discriminate via an injected D-19 control. **D-11's locked mechanism was falsified at plan time and corrected in place**: `test.afterAll` exits 1 but sets `stats.skipped:0` and flips every gated cell to `failed`, destroying the three-way distinction the decision exists to preserve — `globalTeardown` + an `fs` side-channel preserves it, and both exit 1 so exit code alone cannot discriminate them. Wave 0 (`123-01`, `123-02`) is harness-only and must land before any sweep. |
