...
| 126. Page Body and Convex Read Defect Sweep | v15.0 | 0/9 | Planned | - | Created 2026-08-21 by `gsd-sdk query phase.add`, which wrote ONLY the detail section — no checklist entry and no row here. Both added by hand, per the standing note that a phase missing from the checklist makes every counter derived from it structurally wrong. Scope is the seven todos filed at `ec48907e`; five of the six defects are PRE-EXISTING, each proven not-124's by `git log --grep="(124-" -- <file>` returning 0 rather than asserted. |
