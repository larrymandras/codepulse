...
- **Deployment:** CodePulse is live on **Vercel** at `https://codepulse-jade-omega.vercel.app` (Clerk-gated, `pk_test`). It reads the **self-hosted Convex** backend + **Ástríðr**, both over **Tailscale** (`https://lmofficenew.tail5bb6b3.ts.net`, CORS-allowlisted). Ástríðr runs in Docker (`astridr-agent`). Pushes to master auto-deploy.
  - **Corrected 2026-08-05 (Phase 106, DEBT-02), per this repo's Stale Docs rule.** This line previously read "It reads **Convex** (cloud `tidy-whale-981`)". That was true when written (2026-07-08) but went stale at the cloud→self-hosted migration. Evidence: the frontend's `VITE_CONVEX_URL` resolves to the self-hosted tailnet host, confirmed 2026-08-05 (`106-DEBT-VERIFICATION.md` § "DEBT-02 pre-flight amendment", Correction 2); and plan 106-01's sweep found no functional reader of `tidy-whale-981` in either codepulse or astridr-repo. The cloud deployment `tidy-whale-981` has been **retired since 2026-07-15** (its newest telemetry row is frozen at `2026-07-15T00:04:25Z`) and as of this phase is **exported to local disk and pending cancellation**.
