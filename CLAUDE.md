...
- **Which boundary is real — the tailnet or Clerk — is an OPEN question, tracked in `.planning/seeds/SEED-008-convex-auth-posture.md`.** Do not resolve it inline in an unrelated phase, and do not "harden" one module's mutations in isolation: that closes a handful of doors out of ~124 and leaves the codebase inconsistent. Clerk cannot be made fail-closed app-wide today — it is optional by design and the whole Playwright suite depends on `dev:noauth`. Note the container publishes `0.0.0.0:3210-3211` (all interfaces); whether the LAN can actually reach it is unconfirmed (the only Docker inbound firewall rules are **Public**-profile scoped while the LAN interface is **Private**, which points to blocked) — and a probe run from this host cannot settle it, because loopback, Docker-bridge NAT and WSL are each special-cased. Use a device that is neither this machine nor a container on it.
