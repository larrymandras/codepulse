  ...
  return (
    <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <span className="text-base font-mono text-gray-200">{redact(session.sessionId, `S-${session.sessionId.slice(-4)}`)}</span>
        <span
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div>
          <p className="text-sm text-gray-500">CWD</p>
          <p className="text-base font-mono text-gray-200 mt-0.5" title={session.cwd ? maskFilePath(session.cwd) : undefined}>
        ...
        <div>
          ...
          <p className="text-base text-gray-200 mt-0.5">{session.eventCount}</p>
        </div>
      </div>
    </div>
  );
