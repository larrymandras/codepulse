...
export function EntityRow({ icon, primary, secondary, trailing, onClick, wrapPrimary }: EntityRowProps) {
  return (
    <motion.div
      ...
      onClick={onClick}
      className={cn(
        "flex gap-3.5 px-4 py-3 border-b border-border/50 last:border-b-0 transition-all duration-slow ease-house",
        wrapPrimary ? "items-start" : "items-center",
        onClick && "cursor-pointer hover:bg-white/[0.04] hover:pl-6 relative group"
      )}
    >
      {onClick && <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-primary scale-y-0 group-hover:scale-y-100 transition-transform origin-center duration-slow ease-house" />}
