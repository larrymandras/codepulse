...
export default function CronExecutionHistory({ executions }: CronExecutionHistoryProps) {
  return (
    <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
      ...
      {executions.length === 0 ? (
        <p className="text-sm text-gray-500 py-6 text-center">No executions recorded yet</p>
      ) : (
        <div className="max-h-[360px] overflow-y-auto">
          {/* Header */}
          <div className="flex items-center gap-3 px-3 py-1.5 text-[10px] text-gray-500 uppercase tracking-wider border-b border-gray-700/30 sticky top-0 bg-gray-800/80">
          ...
          <div className="space-y-0">
            {executions.map((exec: any, i: number) => (
              <div
                key={exec._id}
                className={`flex items-center gap-3 px-3 py-1.5 text-xs ${i % 2 === 0 ? "bg-gray-800/30" : ""}`}
              >
