...
export function useSavedViews() {
   ...
   * strips entityId (187-05) and entityIds (190-08/D-11/GLXY-04) — a D-09
   * answer-sync id (or id set) is programmatic/transient (re-derived fresh
   * from the latest sync row, never user-chosen) and must never be captured
   * into a saved view; `focus` (entityName) is the durable, shareable
   * identifier for the Entity lens. Generates a fresh shareToken client-side
   * via crypto.randomUUID() (D-03).
   *
  ...
  const saveView = async (
    ...
    // D-06: strip the transient searchQuery before persisting; 187-05/190-08:
    // also strip the transient/programmatic entityId/entityIds.
    const { searchQuery: _sq, entityId: _eid, entityIds: _eids, ...persistable } = filters;
