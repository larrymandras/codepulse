          ...
          <SectionErrorBoundary name="Browser Guard">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gray-800/50 border border-gray-700/50 rounded-lg p-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-1">URLs Blocked</p>
                <p className="text-2xl font-semibold tabular-nums" style={{ color: bgBlocked > 0 ? "var(--status-error)" : undefined }}>
                  {bgBlocked}
                </p>
              </div>
              <div className="bg-gray-800/50 border border-gray-700/50 rounded-lg p-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-1">URLs Allowed</p>
                <p className="text-2xl font-semibold tabular-nums text-gray-100">{bgAllowed}</p>
              </div>
            </div>
            ...
            <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
              ...
              {browserGuardEvents.length === 0 ? (
                ...
                <div className="max-h-[400px] overflow-y-auto">
                  {/* Header */}
                  <div className="grid grid-cols-[120px_1fr_80px_1fr] items-center gap-2 px-3 py-1.5 text-[10px] text-gray-500 uppercase tracking-wider border-b border-gray-700/30 bg-gray-800/80 sticky top-0">
                    <span>Timestamp</span>
                    <span>URL</span>
                    <span>Action</span>
                    <span>Reason</span>
                  ...
                  {browserGuardEvents.map((e: any, i: number) => (
                    <div
                      key={e._id ?? i}
                      className={`grid grid-cols-[120px_1fr_80px_1fr] items-center gap-2 px-3 py-2 text-xs ${i % 2 === 0 ? "bg-gray-800/30" : ""}`}
                    >
                      <span className="text-gray-500 font-mono truncate">
                        {formatRelativeTime(e.timestamp)}
                      </span>
                      <span className="font-mono text-gray-300 truncate" title={e.details?.url ?? e.description}>
                      ...
                      <span className="text-gray-400 truncate">{e.details?.reason ?? "—"}</span>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div className="max-h-[300px] overflow-y-auto">
                  ...
                  {networkPolicyEvents.map((e: any, i: number) => (
                    <div
                      ...
                      className={`grid grid-cols-[120px_1fr_80px_1fr] items-center gap-2 px-3 py-2 text-xs ${i % 2 === 0 ? "bg-gray-800/30" : ""}`}
                    >
                      <span className="text-gray-500 font-mono truncate">{formatRelativeTime(e.timestamp)}</span>
                      <span className="font-mono text-gray-300 truncate">{e.details?.host ?? e.description}</span>
                      <span>
                        <StatusBadge
